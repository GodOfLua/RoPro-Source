...
async def run(message, Arguments, Client, Discord_Bot):
    ...
    try:
        ...
        Role2 = get(message.guild.roles, name="verified")
        if not Role and Role2:
            Role2 = Role
        if not Role and not Role2:
            Role = await message.guild.create_role(name="Verified", hoist=False)
        GuildData["VerifiedRole"] = Role.id
    ...
    if Arguments[1].lower() == "none":
        GuildData["VerifiedRole"] = 0
        SaveData(f"./Data/Server_Data/{str(GuildId)}.json", GuildData)
        await message.channel.send(embed=discord.Embed(
            description = "Verified role has been removed.",
            footer = "Powered by RoPro Verification System · !invite",
            color = 0x3a9518
        ))
        return
    ...
    SaveData(f"./Data/Server_Data/{str(GuildId)}.json", GuildData)
    await message.channel.send(embed=discord.Embed(
        description = role.mention+" has been set as the verified role.",
        footer = "Powered by RoPro Verification System · !invite",
        color = 0x3a9518
    ))