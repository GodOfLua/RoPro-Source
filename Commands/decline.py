...
async def run(message, Arguments, Client, Discord_Bot):
    ...
    res = requests.delete(f"https://groups.roblox.com/v1/groups/{GroupId}/join-requests/users/"+str(res["Id"]), headers={"content-type":"application/json","X-CSRF-TOKEN": x_csrf_token}, cookies={".ROBLOSECURITY": GuildData["BOUND_GROUPS"][GroupId]["COOKIE"]})
    if res.status_code == 200:
        ...
        await mainMSG.edit(embed=discord.Embed(
            description = "Failed to decline user, an API error may have occurred.",
            color = 0xc84c4c
        ))
